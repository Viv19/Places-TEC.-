...
public class Grafo {
    ...
    public void agregarVertice(int v, destino destino) { //RECIBE EL VÉRTICE, FORMADO POR EL INT Y EL DESTINO
        vertices.add(new NodoVertice(v, destino));  //Agrega el vértice al final de la lista de vértices. Su int del vértice y destino
    }

    //Método para agregar una arista al grafo
    public void agregarArista(int origen, int destino, int peso) {
        buscarVertice(origen).aristas.add(new NodoGrafo(destino, peso));  //Buscar el vértice origen y agrega un nodo al final de la lista de aristas de ese vértice
    }
    ...
    public NodoGrafo buscarArista(int origen, int destino) {
    ...
    public void imprimeGrafo() {
        ...
        for (int i = 0; i < this.vertices.size(); i++) {
            ...
            for (int j = 0; j < actual.aristas.size(); j++) {
                System.out.println("Arista de " + actual.vertice +", con destino a "+ actual.destino.datosDestino()
                + " a " + actual.aristas.get(j).vertice + ", con destino a "+ actual.destino.datosDestino()+
                " con peso " + actual.aristas.get(j).peso);
            }
        }
    }
    ...
    public boolean eliminarVertice(int v) {
        ...
        System.out.println("Vértice borrado.");
        return true;
    }
